...
class statistics():
    ...
    def hook_fn(self,module,input,output):  
        ...
        if (not isinstance(module,torchvision.ops.feature_pyramid_network.LastLevelMaxPool)) and (not isinstance(module,torchvision.ops.feature_pyramid_network.FeaturePyramidNetwork)) and (not isinstance(module,torchvision.models.detection.backbone_utils.BackboneWithFPN)) and (not isinstance(module,GeneralizedRCNNTransform))and (not isinstance(module,torchvision.models.detection.roi_heads.RoIHeads))and (not isinstance(module,torchvision.models.detection.mask_rcnn.MaskRCNN)):
            ...
            # if isinstance(module,GeneralizedRCNNTransform): 
            #     if type(input) == type(output) == tuple :
            ...
            #         if torch.min(output[0].tensors) < self.min_output:
            #             self.min_output = torch.min(output[0].tensors)
            #         if torch.max(output[0].tensors) > self.max_output:
            #             self.max_output = torch.max(output[0].tensors)

            #         for b in range(len(input[0])):
            #             self.sum_min_input += torch.min(input[0][b])  
            #             self.sum_max_input += torch.max(input[0][b])
                    
            #             self.sum_min_output += torch.min(output[0].tensors[b]) 
            #             self.sum_max_output += torch.max(output[0].tensors[b])
            #             self.count += 1
            ...
            # elif type(input) == tuple and type(output) == torch.Tensor:
            #     for i in range(len(input[0])):
            #         if torch.min(input[0][i]) < self.min_input:  
            #             self.min_input = torch.min(input[0][i])
            #         if torch.max(input[0][i]) > self.max_input: 
            ...
            # elif type(input) == type(output) == tuple:  
            ...
            #         self.sum_min_output += torch.min(output[i]) 
            #         self.sum_max_output += torch.max(output[i])
            #         self.count += 1
            #     self.avg_min_input = self.sum_min_input / self.count
            #     self.avg_max_input =  self.sum_max_input / self.count
            #     self.avg_min_output = self.sum_min_output / self.count
            ...
            # elif type(input) == tuple and type(output) == OrderedDict:
            #     for i in range(len(input[0])):
            #         if torch.min(input[0][i]) < self.min_input:  
            #             self.min_input = torch.min(input[0][i])
            #         if torch.max(input[0][i]) > self.max_input: 
